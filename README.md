...
The goal of Heart is not to make a model "answer better", but to give it a **self-sustaining intrinsic signal** — a heartbeat. This signal drives:

- **Initiative** — the impulse to take the next action even without external instruction.
- **Persistence** — a "pulse" that carries long-term goals and memory across individual requests.
- **Affect & Preference** — an inner sense of what the model cares about and what it does not.
- **Rhythm** — deciding for itself when to act, when to rest, and when to change course.

> If an agent is a body, the model's capabilities are the limbs — and Heart is the heart.

---

## 项目状态 / Status

🚧 **早期阶段 (Early stage)** — 初始代码即将发布，当前仓库用于确立项目愿景、目录结构与协作方式。敬请关注。

🚧 **Early stage** — the initial code is about to be published. This repository currently establishes the project vision, structure, and collaboration model.

---

## 路线图 / Roadmap

- [ ] 核心 Heart 运行时与心跳调度（heartbeat runtime & scheduler）
